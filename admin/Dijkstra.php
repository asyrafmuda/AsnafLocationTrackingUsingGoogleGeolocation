	...
	function jalurTerpendek($arg_graph, $simpulAwal, $simpulTujuan){
		...
		*/
		...
		for($perulangan = 0; $perulangan < 1; $perulangan++)
		{
			...
			$perbandinganSemuaPerjalanan = array();
			...
			for($perulanganSimpul = 0; $perulanganSimpul < count($simpulYangDikerjakan); $perulanganSimpul++)
			{
				...
				*/
				for($start_baris = 0; $start_baris < $jumlah_baris; $start_baris++)
				{
					...
					$ruas_dan_perjalanan = $graph[ $simpulYangDikerjakan[$perulanganSimpul] ][$start_baris];//pasti berurutan [0][0],[0][1] dst
					
					// PECAH RUAS & BOBOT BERDASARKAN '->'
					// RUAS  : explode[0] = 1
					// BOBOT : explode[1] = 10
					$explode = explode('->', $ruas_dan_perjalanan);
					...
					{						
						...
						* ILUSTRASI MENGUPDATE BOBOT DALAM TABEL :
						  ...
						  | 0->3=40 (45) | <-- bobot diupdate = 45;
						   -------------- 
						*/
						array_push($perjalanan_baris, ($explode[1]+$nilaiSimpulYgDitandai)); // (bobot+0) or (bobot+232)
				...
				if($baris_belum_dikerjakan > 0)
				{
					...
					for($index_perjalanan = 0; $index_perjalanan < count($perjalanan_baris); $index_perjalanan++){
					   if($perjalanan_baris[$index_perjalanan] <= $perjalanan_baris[0]){
						   $perjalanan_baris[0] = $perjalanan_baris[$index_perjalanan];
					   }
					} 

					// BOBOT TERKECIL DARI SETIAP KOLOM SIMPUL
					array_push($perbandinganSemuaPerjalanan, $perjalanan_baris[0]);
			...
			* DAPATKAN 1 BOBOT PALING MINIMUM DARI SIMPUL YG DITANDAI (*)
			  ...
			  | 0->3=40 	| 1->4=20(*) | 2->3=54 |
			   ------------- ------------ --------- 
			*/
			for($min_indexAntarPerjalananYgDitandai = 0; $min_indexAntarPerjalananYgDitandai < count($perbandinganSemuaPerjalanan); $min_indexAntarPerjalananYgDitandai++)
			{
				if($perbandinganSemuaPerjalanan[$min_indexAntarPerjalananYgDitandai] <= $perbandinganSemuaPerjalanan[0]){
					$perbandinganSemuaPerjalanan[0] = $perbandinganSemuaPerjalanan[$min_indexAntarPerjalananYgDitandai];
				}
			}
			...
			foreach($simpulYangDikerjakan as $idx => $v)
			{
				...
				*/
				...
				for($baris1 = 0; $baris1 < $length_baris; $baris1++)
				{
					...
					{
						$perjalanan_baris_dan_ruas1 = $graph[ $simpulYangDikerjakan[$indexAwalAsli] ][$baris1];
						$explode1 = array();
						$explode1 = explode('->', $perjalanan_baris_dan_ruas1);
						if(count($explode1) == 2)
						{
							if($perbandinganSemuaPerjalanan[0] == $explode1[1])
							{
								$dapat_indexAsliPerjalanan = $baris1;
			...
			if($baris_belum_dikerjakan1 > 0){                    
				$graph[$simpul_lama][$dapat_indexAsliPerjalanan] = $graph[$simpul_lama][$dapat_indexAsliPerjalanan] . "->y";
