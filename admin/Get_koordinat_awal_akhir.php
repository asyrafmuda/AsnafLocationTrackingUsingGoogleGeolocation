...
class Get_koordinat_awal_akhir extends DistanceTo{
	...
	*/
	...
	public function Get_simpul($latx, $lngx, $id_buang)
	{
		...
		$select = mysqli_query($this->koneksi, "SELECT * FROM graph where id not in($id_buang) and simpul_awal != '' and simpul_tujuan != '' and jalur != '' and perjalanan != ''");
		...
		for($index_obj = 0; $kolom = mysqli_fetch_array($selectRow, MYSQLI_ASSOC); $index_obj++){
			...
			// CARI perjalanan yg paling dekat
			...
			$list['perjalanan']		= $jarakUserKeKoordinatSimpul[0];
			$list['nodes']			= $dataNode;
			$list['count_koordinat']= $countCoordinate;

			// Create json
			// example output : 
			// {"0" : [{"row_id":17, "index":"7", "perjalanan":"427.66", "count_koordinat":"15", "nodes":"0-1"}]}
			$obj->$index_obj = [$list];
