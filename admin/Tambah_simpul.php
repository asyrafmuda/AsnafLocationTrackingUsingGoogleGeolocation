<?php
include "Count_Perjalanan_Tambah_Simpul.php";

class Tambah_simpul extends Count_Perjalanan_Tambah_Simpul{
	...
	*/
	...
	public function dobelSimpul($nodes0, $nodes1, $index_koordinat_json, $graph){
		...
		$node0				= $nodeSplit[0];
		$node1				= $nodeSplit[1];
		$dataPerjalanan		= $jsonOneRoute['distance_metres'][0];
		...
		// @extends
		$perjalanan 	= $this->Count_Perjalanan_Tambah_Simpul(0, $limit, $jArrCoordinates); // 0, koordinat tengah, jSON coordinates
		
		//replace array graph[5][0] = 6->888.6
		$graph[$baris][$kolom] = ($max_simpul_db+1) . "->" . $perjalanan;				
		
		// buat dan simpan (new record) json koordinat yang baru ke DB
		$start_loop = 0;
		// @local
		$this->createAndSave_NewJsonCoordinate($start_loop, $limit, $jArrCoordinates, $nodes0, ($max_simpul_db + 1), $perjalanan);
		...
		// @extends
		$perjalanan 	= $this->Count_Perjalanan_Tambah_Simpul($index_koordinat_json, $limit1, $jArrCoordinates); // coordinate tengah sampai akhir
		
		
		// new array graph[6][0] = 4->777.4
		$graph[($max_simpul_db+1)][0] = $nodes1 . "->" . $perjalanan; //didefinisikan [0] karena index baru di graph[][]
		
		// buat dan simpan (new record) json koordinate yang baru ke DB
		$this->createAndSave_NewJsonCoordinate($start_loop1, $limit1, $jArrCoordinates, ($max_simpul_db + 1), $nodes1, $perjalanan);
		...
		$node01				= $nodeSplit1[0];
		$node11				= $nodeSplit1[1];
		$dataPerjalanan1	= $jsonOneRoute1['distance_metres'][0];
		...
		// @extends
		$perjalanan = $this->Count_Perjalanan_Tambah_Simpul(0, $index_dobel_koordinat_json, $jArrCoordinates1); // 0, koordinat awal ke tengah, JSONArray coordinate

		//replace array graph[4][0] = 6->777.4
		$graph[$baris1][$kolom1] = ($max_simpul_db+1) . "->" . $perjalanan;
		
		
		// buat dan simpan (new record) json koordinate yang baru ke DB
		$start_loop2 = 0;
		// @local
		$this->createAndSave_NewJsonCoordinate($start_loop2, $index_dobel_koordinat_json, $jArrCoordinates1, $baris1, ($max_simpul_db + 1), $perjalanan);
		...
		// @extends
		$perjalanan 	= $this->Count_Perjalanan_Tambah_Simpul($index_dobel_koordinat_json, $limit2, $jArrCoordinates1); // koordinat tengah sampai akhir
		
		//replace array graph[6][1] = 5->888.6
		$graph[($max_simpul_db+1)][1] = $t_nodes1 . "->" . $perjalanan; // didefinisikan [1] karena sdh ada index 0 di graph[][]
		
		// buat dan simpan (new record) json koordinate yang baru ke DB
		// @local
		$this->createAndSave_NewJsonCoordinate($index_dobel_koordinat_json, $limit2, $jArrCoordinates1, ($max_simpul_db + 1), $t_nodes1, $perjalanan);		
	 ...
	 *  no return
	 */
	public function createAndSave_NewJsonCoordinate($mulai, $limit, $jArrCoordinates, $field_simpul_awal, $field_simpul_akhir, $new_perjalanan){
		...
		*/
		...
		$json_baru['distance_metres'] = [$new_perjalanan];
		...
		try {			
			...
			$sql = "INSERT INTO graph values(($new_id[newid]+1), '$field_simpul_awal', '$field_simpul_akhir', '$jalur_baru', '$new_perjalanan', 'Y')";
